...
docker compose up --build            # поднимает postgres + django
...
Запускаются 16 тестов на ключевые правила: запись на тренировки, абонементы,
сигналы, декораторы ролей.
...
## Документация для автора

В папке [`docs/`](./docs) лежат пояснения простыми словами:

- [`УСТРОЙСТВО_ПРОЕКТА.md`](./docs/УСТРОЙСТВО_ПРОЕКТА.md) — что за файлы, как всё работает, поток данных от запроса до ответа.
- [`ВОПРОСЫ_К_ЗАЩИТЕ.md`](./docs/ВОПРОСЫ_К_ЗАЩИТЕ.md) — типовые вопросы комиссии и готовые ответы.

## План разработки

- [x] Неделя 1 — модели, миграции, админ-панель, аутентификация, роли, демо-данные.
- [x] Неделя 2 — шаблоны и формы: расписание, запись на тренировки, покупка абонемента, рабочий стол тренера.
- [x] Неделя 3 — отчёты (посещаемость, выручка, загрузка тренеров), экспорт PDF/Excel.
- [x] Неделя 4 — тесты, Docker, PostgreSQL, CI.
