...
def class_detail(request, pk):
    ...
    if request.user.is_authenticated and request.user.role == 'client':
        ...
        if client:
            ...
            can_enroll, reason = can_enroll_check(client, tc, enrollment)

    user = request.user
    can_see_list = (
        user.is_authenticated
        and (user.is_superuser or user.role == 'admin'
             or (user.role == 'trainer' and tc.trainer_id and tc.trainer.user_id == user.id))
    )
    enrollments = tc.enrollments.select_related('client__user').all() if can_see_list else []
...
def can_enroll_check(client, tc, enrollment):
...
def enroll(request, pk):
    ...
    can, reason = can_enroll_check(client, tc, existing)
    if not can:
...
@trainer_required
def trainer_class_detail(request, pk):
...
@trainer_required
@require_POST
def mark_class_status(request, pk):
    trainer = request.user.trainer_profile
    tc = get_object_or_404(TrainingClass, pk=pk, trainer=trainer)
...
@trainer_required
@require_POST
def mark_enrollment_status(request, enrollment_id):
    trainer = request.user.trainer_profile
    enrollment = get_object_or_404(
    ...
    if new_status in valid:
        ...
        enrollment.save(update_fields=['status'])
        # если клиент пришёл — увеличиваем счётчик посещений в его активном абонементе
        if new_status == 'attended':
            date = enrollment.training_class.start_time.date()
    ...
    return redirect('schedule:trainer_class_detail', pk=enrollment.training_class_id)
